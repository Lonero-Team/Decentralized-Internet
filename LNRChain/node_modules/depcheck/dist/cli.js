...
function cli(args, log, error, exit) {
  var opt = (0, _yargs2.default)(args).usage('Usage: $0 [DIRECTORY]').boolean(['dev', 'ignore-bin-package', 'skip-missing']).default({
    dev: true,
  ...
  checkPathExist(rootDir, 'Path ' + dir + ' does not exist').then(function () {
    ...
    return exit(opt.argv.json || noIssue(result) ? 0 : -1);
